    ...
    return new Promise((success, failure) => {
      ...
      fetch(this.url + this.page).then((html) => html.text()).then((htmlText) => {
        ...
        if (items["0"].rawText.includes('Sorry')) {
          success([{name: this.url.replace(GoGoAnime + MajorLink.Search, '').replace('&page=', ''), info: 'Google', link: 'Error'}, 1]);
        }
        ...
        for (var i = 0; i < length; i++) {
          ...
          var animeLink = GoGoAnime + animeImage.childNodes[1].attributes.href;
